...
import com.pdev.rempms.documentservice.constants.AuthErrorMessages;
import com.pdev.rempms.documentservice.constants.ClaimsConstant;
import com.pdev.rempms.documentservice.constants.RolePermissionsConstants;
import com.pdev.rempms.documentservice.exception.UnauthorizedException;
import com.pdev.rempms.documentservice.service.rest.kcs.KeyCloakClientService;
                ...
                if (jwtUtil.hasRecruitmentPermission(azpWithRoles)) {
                    ...
                    for (Map.Entry<String, List<String>> azpWithRole : azpWithRoles.entrySet()) {
                        if (azpWithRole.getKey().equals(RolePermissionsConstants.CLIENT_ID_REM_PMS) &&
                                azpWithRole.getValue().contains(RolePermissionsConstants.PERMISSION_DOCUMENT_SERVICE)) {

                            grantedAuthorities.add(new SimpleGrantedAuthority(RolePermissionsConstants.PERMISSION_DOCUMENT_SERVICE));
                        }
                    }
