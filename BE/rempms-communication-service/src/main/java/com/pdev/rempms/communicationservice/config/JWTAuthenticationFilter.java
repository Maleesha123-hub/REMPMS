...
import com.pdev.rempms.communicationservice.constants.AuthErrorMessages;
import com.pdev.rempms.communicationservice.constants.ClaimsConstant;
import com.pdev.rempms.communicationservice.constants.RolePermissionsConstants;
import com.pdev.rempms.communicationservice.exception.UnauthorizedException;
import com.pdev.rempms.communicationservice.service.rest.kcs.KeyCloakClientService;
            ...
            if (Boolean.TRUE.equals(valid)) {
                ...
                if (jwtUtil.hasRecruitmentPermission(azpWithRoles)) {
                    ...
                    for (Map.Entry<String, String> roleWithScope : userRolesWithScope.entrySet()) {
                        if (roleWithScope.getKey().equals(RolePermissionsConstants.APP_SCOPE_COMMUNICATION_SERVICE)) {
                    ...
                    for (Map.Entry<String, List<String>> azpWithRole : azpWithRoles.entrySet()) {
                        if (azpWithRole.getKey().equals(RolePermissionsConstants.CLIENT_ID_REM_PMS) &&
                                azpWithRole.getValue().contains(RolePermissionsConstants.PERMISSION_COMMUNICATION_SERVICE)) {

                            grantedAuthorities.add(new SimpleGrantedAuthority(RolePermissionsConstants.PERMISSION_COMMUNICATION_SERVICE));
                        }
                    }
