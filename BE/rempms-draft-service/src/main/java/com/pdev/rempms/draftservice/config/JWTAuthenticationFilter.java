...
import com.pdev.rempms.draftservice.constants.AuthErrorMessages;
import com.pdev.rempms.draftservice.constants.ClaimsConstant;
import com.pdev.rempms.draftservice.constants.RolePermissionsConstants;
import com.pdev.rempms.draftservice.exception.UnauthorizedException;
import com.pdev.rempms.draftservice.service.rest.KeyCloakClientService;
                ...
                if (jwtUtil.hasRecruitmentPermission(azpWithRoles)) {
                    ...
                    for (Map.Entry<String, List<String>> azpWithRole : azpWithRoles.entrySet()) {
                        if (azpWithRole.getKey().equals(RolePermissionsConstants.CLIENT_ID_REM_PMS) &&
                                azpWithRole.getValue().contains(RolePermissionsConstants.PERMISSION_DRAFT_SERVICE)) {

                            grantedAuthorities.add(new SimpleGrantedAuthority(RolePermissionsConstants.PERMISSION_DRAFT_SERVICE));
                        }
                    }
